...
final class AdminController extends Controller {
 private function allow(string $permission):void {if(!Gate::allows($permission))throw HttpException::forbidden();}
  public function index(Request $r):Response {
   ...
   $counts=['Properties'=>DB::scalar('SELECT COUNT(*) FROM properties'),'Bookings'=>DB::scalar('SELECT COUNT(*) FROM bookings'),'Pending properties'=>DB::scalar("SELECT COUNT(*) FROM properties WHERE status='pending'"),'Published properties'=>DB::scalar("SELECT COUNT(*) FROM properties WHERE status='published'"),'Active hosts'=>DB::scalar("SELECT COUNT(*) FROM users WHERE role='host' AND status='active'"),'Active guests'=>DB::scalar("SELECT COUNT(*) FROM users WHERE role='guest' AND status='active'"),'Confirmed bookings'=>DB::scalar("SELECT COUNT(*) FROM bookings WHERE status='confirmed'"),'Cancelled bookings'=>DB::scalar("SELECT COUNT(*) FROM bookings WHERE status='cancelled'")];
   ...
   case 'property':
    ...
    DB::transaction(function()use($r,$id,$actor){
     ...
     if($r->string('status')==='published'){
      if($p['status']!=='pending')throw new BusinessException('A submitted property is required.');
