...
use App\Core\{Config,Database as DB,BusinessException};
use App\Models\User;
final class AccountService {
 ...
 public static function reset(string $token,string $password):void {
  ...
  DB::transaction(function()use($token,$password){
   ...
   $user=User::findByEmail((string)$r['email']);
   if($user) DB::execute('DELETE FROM user_sessions WHERE user_id=?',[(int)$user['id']]);
 ...
 public static function verification(int $id):void {
  ...
  $token=bin2hex(random_bytes(32));
  $ttl=max(1,Config::int('auth.email_verification_ttl_hours',48));
  DB::transaction(function()use($id,$u,$token,$ttl){
   DB::execute('DELETE FROM email_verifications WHERE user_id=? AND used_at IS NULL',[$id]);
   DB::insert('email_verifications',['user_id'=>$id,'token_hash'=>hash('sha256',$token),'expires_at'=>gmdate('Y-m-d H:i:s',time()+($ttl*3600))]);
   MailService::queue($u['email'],'Verify your StayIn email',"Welcome to StayIn!\n\nVerify your email address to activate protected account features:\n".url('/verify-email/'.$token)."\n\nThis single-use link expires in {$ttl} hours. If you did not create this account, you can ignore this email.");
  });
 }
 public static function verify(string $token):void {
  DB::transaction(function()use($token){
   ...
   DB::update('email_verifications',['used_at'=>gmdate('Y-m-d H:i:s')],['user_id'=>$v['user_id']]);
  });
 }
}
